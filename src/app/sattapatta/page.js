...
const Products = () => {
  ...
  return (
    <div className="min-h-screen w-full px-[3vw] py-[3vh] bg-[#F5F5F7] dark:from-[#1c1c1e] dark:to-[#2e2e30]">
  ...
  <div className="relative">
    <Link href="/sattapatta/offers">
      <button
        className="text-xs sm:text-sm px-2 py-1 bg-[#007BFF] text-[#1A1A1A] rounded-xl hover:bg-[#0056D2] transition-all whitespace-nowrap"
      >
  ...
  <button
    ...
    className="text-xs sm:text-sm px-2 py-1 bg-[#007BFF] text-[#1A1A1A] rounded-xl hover:brightness-110 transition-all whitespace-nowrap"
  >
    ➕ Add Sattapatta Item
  </button>
</div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 rounded-xl">
          {products.length === 0 ? (
            <p className="text-center text-lg text-[#1A1A1A] dark:text-gray-300">No products available.</p>
          ) : (
              ...
              <div
                ...
                className={`bg-gradient-to-b from-[#F5F5F7] to-[#d3e1ef] py-[1vw] px-[1vw] rounded-xl border-2 border-[#D1D1D1] shadow-lg transform transition-all duration-300 relative flex flex-col gap-3 dark:bg-gradient-to-tl dark:from-[#b4b0b0] dark:to-[#504e4e] ${
                  ...
                  product.status === 'locked'
                    ? 'border-4 border-double shadow-[#82e677] border-[#D1D1D1]'
                    : product.status === 'ongoing'
                    ? 'border-4 border-double shadow-lg shadow-[#f444fa] border-[#D1D1D1]'
                    : ''
                }`}
  ...
  {inExchangeItemIds.has(product._id) && (
    <div className="absolute inset-0 bg-black bg-opacity-60 flex flex-col items-center justify-center rounded-lg z-20">
      ...
      <p className="text-[#D1D1D1] font-bold text-sm select-none">Booked</p>
    </div>
  )}
    ...
    {userId &&
      ...
      userId === (typeof product.owner === 'string' ? product.owner : product.owner._id) && (
        <button
          ...
          className="w-auto bg-[#007BFF] text-xs font-medium px-2 py-1 text-[#1A1A1A] rounded-md hover:bg-[#0056D2] hover:text-black transition duration-300 mr-2"
        >
          Exchange
        </button>
      )}
