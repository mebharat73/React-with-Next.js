  ...
  return (
    <div className="flex items-center justify-center flex-col">
      ...
      <form onSubmit={updateProfile} className="flex items-center mt-3">
        <input
          type="file"
          className="border border-[#8b2fa2] h-10 rounded-xl px-5 text-[#68217A] text-base font-bold  py-1 w-full shadow-md dark:text-white dark:bg-zinc-600"
          id="profile-image"
          onChange={(e) => {
            const files = [];
            const urls = [];
        ...
        <button
          ...
          type="submit"
          className="bg-[#68217A] text-[#C3EF38] px-7 py-2 h-8 ml-3 rounded-2xl cursor-pointer disabled:bg-[#8b2fa2] disabled:cursor-not-allowed flex items-center disabled:bg-opacity-80 hover:bg-[#8b2fa2]"
        >
          {uploading ? (
            <>
              <span>Uploading</span>
